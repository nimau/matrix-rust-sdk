...
use matrix_sdk::{
    ...
    Client, ClientBuildError, Result, ServerName, Session,
};
...
impl OidcCli {
    ...
    async fn new(data_dir: &Path, session_file: PathBuf) -> anyhow::Result<Self> {
        ...
        let user_session = cli.client.session().expect("A logged-in client should have a session");
    ...
    async fn whoami(&self) {
        ...
        let issuer =
            client.authentication_issuer().await.expect("A logged in OIDC client has an issuer");
    ...
    async fn update_stored_session(&self) -> anyhow::Result<()> {
        ...
        let user_session = self.client.session().expect("A logged in client has a session");
    ...
    loop {
            ...
            .await
        {
            Ok(client) => {
                ...
                if let Some(issuer) = client.authentication_issuer().await {
