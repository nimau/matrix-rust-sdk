...
impl Rules {
    ...
    pub(crate) fn is_user_mention_enabled(&self) -> bool {
        ...
        #[allow(deprecated)]
        if let Some(rule) = self.ruleset.get(RuleKind::Override, PredefinedOverrideRuleId::ContainsDisplayName) {
        ...
        #[allow(deprecated)]
        if let Some(rule) = self.ruleset.get(RuleKind::Content, PredefinedOverrideRuleId::ContainsDisplayName) {
            if rule.enabled() && rule.actions().iter().any(|a| a.should_notify()) {
                return true;
            }
        }
    ...
    /// Sets whether a rule is enabled.
    ///
    /// # Arguments
    ///
    /// * `scope` - A `RuleScope`
